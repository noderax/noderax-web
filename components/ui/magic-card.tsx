...
export function MagicCard(props: MagicCardProps) {
  const {
    ...
    className,
    gradientSize = 220,
    gradientColor = "rgba(127, 29, 29, 0.18)",
    gradientOpacity = 0.72,
    gradientFrom = "rgba(248, 113, 113, 0.9)",
    gradientTo = "rgba(127, 29, 29, 0.3)",
    mode = "gradient",
  } = props;

  const glowFrom = isOrbMode(props)
    ? props.glowFrom ?? "rgba(239, 68, 68, 0.48)"
    : "rgba(239, 68, 68, 0.48)";
  const glowTo = isOrbMode(props)
    ? props.glowTo ?? "rgba(127, 29, 29, 0.18)"
    : "rgba(127, 29, 29, 0.18)";
  const glowAngle = isOrbMode(props) ? props.glowAngle ?? 90 : 90;
  const glowSize = isOrbMode(props) ? props.glowSize ?? 420 : 420;
  const glowBlur = isOrbMode(props) ? props.glowBlur ?? 60 : 60;
  const glowOpacity = isOrbMode(props) ? props.glowOpacity ?? 0.75 : 0.75;
  ...
  const reset = useCallback(
    (reason: ResetReason = "leave") => {
      ...
      const offscreen = -gradientSizeRef.current;
  ...
  return (
    <motion.div
      ...
      style={{
        background: useMotionTemplate`
          linear-gradient(var(--surface-panel) 0 0) padding-box,
    ...
    >
      <div className="absolute inset-px z-20 rounded-[inherit] bg-[var(--surface-panel)]" />
